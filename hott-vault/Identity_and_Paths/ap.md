...
**证明**: 通过路径归纳。假设 $p$ 是 $\mathsf{refl}_x$，此时定义 $\mathsf{ap}_f(p) \defeq \mathsf{refl}_{f(x)} : f(x) = f(x)$。
...
$\mathsf{ap}_f$ 是非依赖版本。对于依赖函数 $f : \prod_{x:A} P(x)$，需要更精细的**依赖映射** $\mathsf{apd}_f$，见 [[02_transport|类型族是纤维化]]。
...
- [[02_transport|Transport and apd / 传输与依赖映射]]
